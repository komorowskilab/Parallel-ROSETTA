...
// Revisions.....: Aš 0797: Deals with comment lines and blank lines.
//                 Aš 0398: Decomposed, added comma as a token delimiter.
//                 Aš 0698: Should handle import of export format.
//                 Aš 0401: Handles explicitly supplied dimensions.
//===================================================================

Structure *
MyDecisionTableImporter::Apply(Structure &structure) const {
	...
	message.Notify("Importing attribute names...");

	// Import attribute names.
	if (!ImportNames(stream, names, line_no))
		return NULL;
	...
	// Dimensions explicitly supplied? If so, get these and import names again.
	if (names.size() == 2 && names[0].IsInteger() && names[1].IsInteger()) {
		message.Notify("Supplied table dimensions (" + names[0] + ", " + names[1] + ") detected...");
		message.Notify("Importing attribute names...");
		dimensions_supplied    = true;
		no_objects_supplied    = names[0].GetInteger();
		no_attributes_supplied = names[1].GetInteger();
		if (!ImportNames(stream, names, line_no))
			return NULL;
	}

	message.Notify("Importing attribute types...");

	// Import attribute types.
	if (!ImportTypes(stream, types, exponents, line_no, names.size()))
		return NULL;
...
bool
MyDecisionTableImporter::GetTokens(const String &line, Vector(String) &tokens) const {
...
bool
MyDecisionTableImporter::ImportNames(ifstream &stream, Vector(String) &names, int &line_no) const {
...
bool
MyDecisionTableImporter::ImportTypes(ifstream &stream, Vector(Attribute::Type) &types, Vector(int) &exponents, int &line_no, int no_attributes) const {
...
bool
MyDecisionTableImporter::ImportData(ifstream &stream, DecisionTable &table, bool masked, int &line_no) const {
...
bool
MyDecisionTableImporter::InitializeTable(DecisionTable &table, bool masked, const Vector(String) &names, const Vector(Attribute::Type) &types, const Vector(int) &exponents) const {
...
MyDecisionTableImporter::Clone() {
	return new MyDecisionTableImporter;
}
