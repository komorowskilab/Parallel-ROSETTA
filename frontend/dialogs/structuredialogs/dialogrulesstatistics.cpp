...
CDialogRulesStatistics::CDialogRulesStatistics(CWnd* pParent /*=NULL*/)
...
void CDialogRulesStatistics::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDialogRulesStatistics, CDialog)
...
BOOL CDialogRulesStatistics::OnInitDialog() 
{
...
void CDialogRulesStatistics::CalculateStatistics()
{
  ...
  if (maximum_length == Undefined::Integer())
	...
	Rules::DVMap::iterator iterator = descriptors.begin();
  ...
  iterator = descriptors.begin();
	...
	while (!(iterator == descriptors.end())) {
		...
		int value               = (*iterator).first.second;
...
void CDialogRulesStatistics::SetRules(Rules *rules)
{
	m_Rules = rules;
}
