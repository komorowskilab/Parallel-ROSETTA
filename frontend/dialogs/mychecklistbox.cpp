...
#include <kernel\basic\algorithm.h> // For std::max.
...
void CMyCheckListBox::SetTabStops()
{
	delete []m_lpnEachStop;
...
BOOL CMyCheckListBox::SetTabStops( const int& cxEachStop )
{
	if(cxEachStop < 0)
		return FALSE;
	delete []m_lpnEachStop;
...
BOOL CMyCheckListBox::SetTabStops( int nTabStops, LPINT rgTabStops )
{
	delete []m_lpnEachStop;
	int nStop = 0;
	...
	{
		nStop = (rgTabStops[i] * nBaseUnit) / 4;
		if(nStop > nPrevStop)
		{
...
int CMyCheckListBox::GetAverageCharWidths()
{
	CFont* pFont = GetFont();
	LOGFONT lf;
	pFont->GetLogFont(&lf);
