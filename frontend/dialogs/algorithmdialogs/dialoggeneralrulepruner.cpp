...
/////////////////////////////////////////////////////////////////////////////
// CDialogGeneralRulePruner dialog


CDialogGeneralRulePruner::CDialogGeneralRulePruner(CWnd* pParent /*=NULL*/)
	: CDialogAlgorithm(CDialogGeneralRulePruner::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialogGeneralRulePruner)
	m_InvertLogic = FALSE;
	m_ProbLower = 0.0f;
	m_ProbUpper = 0.0f;
	m_SuppLower = 0;
...
void CDialogGeneralRulePruner::DoDataExchange(CDataExchange* pDX)
{
	CDialogAlgorithm::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialogGeneralRulePruner)
...
BEGIN_MESSAGE_MAP(CDialogGeneralRulePruner, CDialogAlgorithm)
	//{{AFX_MSG_MAP(CDialogGeneralRulePruner)
...
/////////////////////////////////////////////////////////////////////////////
// CDialogGeneralRulePruner message handlers
...
BOOL CDialogGeneralRulePruner::OnInitDialog() 
{
	CDialogAlgorithm::OnInitDialog();
	
	if (m_Algorithm.IsNULL() || !m_Algorithm->IsA(MYGENERALRULEFILTER)) {
		Message::Error("Algorithm not set or of incorrect type.");
		EndDialog(IDABORT);
		return FALSE;
	}

	if (m_Structure.IsNULL() || !m_Structure->IsA(GENERALRULES)) {
	...
	Handle<MyGeneralRuleFilter> pruner = dynamic_cast(MyGeneralRuleFilter *, m_Algorithm.GetPointer());

	// Get the rule set to prune.
	Handle<GeneralRules> rules = dynamic_cast(GeneralRules *, m_Structure.GetPointer());
	...
	if (!has_decision_attribute) {
		...
		pruner->SetFilteringStrategy(pruner->GetFilteringStrategy() & ~MyGeneralRuleFilter::FILTERING_RHS_SUPPORT);
		pruner->SetFilteringStrategy(pruner->GetFilteringStrategy() & ~MyGeneralRuleFilter::FILTERING_RHS_ACCURACY);
		pruner->SetFilteringStrategy(pruner->GetFilteringStrategy() & ~MyGeneralRuleFilter::FILTERING_RHS_COVERAGE);
		pruner->SetFilteringStrategy(pruner->GetFilteringStrategy() & ~MyGeneralRuleFilter::FILTERING_RHS_STABILITY);
		pruner->SetFilteringStrategy(pruner->GetFilteringStrategy() & ~MyGeneralRuleFilter::FILTERING_DECISION);
	...
	m_RadioAnd.SetCheck(pruner->GetFilteringConnective() == MyGeneralRuleFilter::CONNECTIVE_AND);
	m_RadioOr.SetCheck(pruner->GetFilteringConnective() == MyGeneralRuleFilter::CONNECTIVE_OR);

	// Set default check button states.
	m_CheckSupp.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_RHS_SUPPORT);
	m_CheckProb.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_RHS_ACCURACY);
	m_CheckCoverage.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_RHS_COVERAGE);
	m_CheckStab.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_RHS_STABILITY);
	m_CheckDec.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_DECISION);
	m_CheckDominating.SetCheck(pruner->IsDominatingDecision());
	m_CheckLength.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_LHS_LENGTH);
	m_CheckValue.SetCheck(pruner->GetFilteringStrategy() & MyGeneralRuleFilter::FILTERING_CONDITION);
...
void CDialogGeneralRulePruner::OnOK() 
{
	...
	if (m_CheckSupp.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_RHS_SUPPORT;
	if (m_CheckProb.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_RHS_ACCURACY;
	if (m_CheckCoverage.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_RHS_COVERAGE;
	if (m_CheckStab.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_RHS_STABILITY;
	if (m_CheckDec.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_DECISION;
	if (m_CheckLength.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_LHS_LENGTH;
	if (m_CheckValue.GetCheck())
		strategy |= MyGeneralRuleFilter::FILTERING_CONDITION;
	...
	if (m_RadioAnd.GetCheck())
		pruner->SetFilteringConnective(MyGeneralRuleFilter::CONNECTIVE_AND);
	else if (m_RadioOr.GetCheck())
		pruner->SetFilteringConnective(MyGeneralRuleFilter::CONNECTIVE_OR);
	else
...
void CDialogGeneralRulePruner::OnCheckDecision() 
{
...
void CDialogGeneralRulePruner::OnCheckAccuracy() 
{
...
void CDialogGeneralRulePruner::OnCheckCoverage() 
{
...
void CDialogGeneralRulePruner::OnCheckStability() 
{
...
void CDialogGeneralRulePruner::OnCheckSupport() 
{
...
void CDialogGeneralRulePruner::OnCheckLength() 
{
...
void CDialogGeneralRulePruner::OnCheckValue() 
{
...
void CDialogGeneralRulePruner::OnSelchangeComboAttribute() 
{
